...
<!--/Pre Loader -->
        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">隔离信息管理管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">隔离信息管理管理</li>
                            <li class="breadcrumb-item active">隔离信息管理列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">隔离信息管理列表</h3>
                            <div class="table-responsive mb-3" id="tableDiv">
                                <div class="col-sm-12">
                                                                                                             
                                        <label>健康码打卡</label>
                                        <input type="text" id="dakaNameSearch" placeholder="请输入健康码打卡" aria-controls="tableId" class="form-control form-control-sm">
                                        ...
                                        <span class="导入导出">
                                            导入<input name="file" id="dakaExcelFile" type="file" class="btn btn-success" style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success" onclick="download('/upload/dakaMuBan.xls')">导入模板下载</button>
                                        </span>
                                ...
                                <table id="tableId" class="table table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <th onclick="sort('daka_name')">健康码打卡<i id="dakaNameIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('daka_file')">健康码照片<i id="dakaFileIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('daka_wendu')">体温<i id="dakaWenduIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('daka_text')">备注<i id="dakaTextIcon" class="fa fa-sort"></i></th>
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //健康码打卡
            var dakaNameSearchInput = $('#dakaNameSearch');
            if( dakaNameSearchInput != null){
                if (dakaNameSearchInput.val() != null && dakaNameSearchInput.val() != '') {
                    searchForm.dakaName = $('#dakaNameSearch').val();
                }
            }
                                    getDataList();
        }
        ...
        function setDataRow(item, number) {
            ...
            //健康码打卡
            var dakaNameCell = document.createElement('td');
            dakaNameCell.innerHTML = item.dakaName;
            row.appendChild(dakaNameCell);


                //健康码照片
            var dakaFileCell = document.createElement('td');
            if(item.dakaFile ==null || item.dakaFile == '' || item.dakaFile == 'null'){
                    dakaFileCell.innerHTML = '没有下载文件';
            }else {
                var dakaFileFile = document.createElement('button');
                    dakaFileFile.innerHTML = '下载';
                var dakaFileFileAttr = "download('" + item.dakaFile + "')"
                    dakaFileFile.setAttribute('onclick',dakaFileFileAttr);
                    dakaFileCell.appendChild(dakaFileFile);
            }
            row.appendChild(dakaFileCell);


            //体温
            var dakaWenduCell = document.createElement('td');
            dakaWenduCell.innerHTML = item.dakaWendu;
            row.appendChild(dakaWenduCell);


            //备注
            var dakaTextCell = document.createElement('td');
            dakaTextCell.innerHTML = item.dakaText;
            row.appendChild(dakaTextCell);
        ...
        function upload() {
            /*导入数据*/
            $('#dakaExcelFile').fileupload({
                url: baseUrl + 'file/upload',
                headers: {token: window.sessionStorage.getItem("token")},
                dataType: "json",
                done: function (e, data) {
                    http("daka/batchInsert?fileName="+data.result.file, "get", {} , (res) => {
                        if(res.code == 0){
