    ...
    <div class="left_nav">
        ...
        <ul>
            <li  v-for="(item,index) in centerMenu" v-bind:key="index" @click="jump(item.url)"
                 :class="item.url=='../daka/list.jsp'?'onclickbiaoqian':''"><i class="layui-icon">&#xe6b1;</i>{{item.name}}</li>
        </ul>
    </div>
</div>        <!-- 个人中心菜单 -->
...
<!--                <button @click="jump('../daka/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加隔离信息管理信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        <td>{{item.dakaName}}</td>
                        <td v-if="item.dakaFile" @click="jump(baseUrl+item.dakaFile)" style="color: red">
                            健康码照片下载
                        </td>
                        <td v-else>
                            无健康码照片
                        </td>
                        <td>{{item.dakaWendu}}</td>
                        <td><button v-if="item.dakaText!=null&&item.dakaText!=''" @click="showContentFunction(item.dakaText)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../daka/add.jsp?dakaId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.dakaTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`daka/update`, 'post', {
                    id:id,
//                    dakaTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`daka/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/daka/list2.jsp")
        ...
        // var id = http.getParam('id');
            ...
            function pageList() {
                // 获取列表数据
                http.request('daka/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('daka/list', 'get', vue.searchForm, function (res1) {
