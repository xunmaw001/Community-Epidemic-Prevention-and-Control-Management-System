    ...
    <div class="left_nav">
        ...
        <ul>
            <li  v-for="(item,index) in centerMenu" v-bind:key="index" @click="jump(item.url)"
                 :class="item.url=='../fengkong/list.jsp'?'onclickbiaoqian':''"><i class="layui-icon">&#xe6b1;</i>{{item.name}}</li>
        </ul>
    </div>
</div>        <!-- 个人中心菜单 -->
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.fengkongTypes=="" || searchForm.fengkongTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in fengkongTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.fengkongTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../fengkong/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加高风险地区记录信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.fengkongName}}</td>
                        <td><img :src="baseUrl+item.fengkongPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.fengkongDidianValue}}</td>
                        <td>{{item.fengkongValue}}</td>
                        <td><button v-if="item.fengkongContent!=null&&item.fengkongContent!=''" @click="showContentFunction(item.fengkongContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../fengkong/add.jsp?fengkongId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.fengkongTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`fengkong/update`, 'post', {
                    id:id,
//                    fengkongTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`fengkong/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/fengkong/list2.jsp")

        // var id = http.getParam('id');

        //风险类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.fengkongTypes = $(this).attr('index');
            pageList();
        });
           ...
           function fengkongDidianTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fengkong_didian_types", 'get', {}, function (res) {
                   if(res.code == 0){
                       vue.fengkongDidianTypesList = res.data.list;
                   }
               });
           }
           //当前表的 风险类型 字段 字典表查询方法
           function fengkongTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fengkong_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            function pageList() {
                // 获取列表数据
                http.request('fengkong/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('fengkong/list', 'get', vue.searchForm, function (res1) {
