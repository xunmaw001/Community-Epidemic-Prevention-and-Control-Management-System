...
public class ForumController {
    ...
    @Autowired
    private SheguanService sheguanService;//社管员
    @Autowired
...
, "usersId"
, "sheguanId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 社管员
            //级联表
            SheguanEntity sheguan = sheguanService.selectById(forum.getSheguanId());
            if(sheguan != null){
            BeanUtils.copyProperties( sheguan , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"
, "usersId"
, "sheguanId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setSheguanId(sheguan.getId());
            }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setSheguanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("sheguan_id", forum.getSheguanId())
...
//                            forumEntity.setSheguanId(Integer.valueOf(data.get(0)));   //社管员 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    SheguanEntity sheguan = sheguanService.selectById(forum.getSheguanId());
                if(sheguan != null){
                    BeanUtils.copyProperties( sheguan , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setSheguanId(sheguan.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
